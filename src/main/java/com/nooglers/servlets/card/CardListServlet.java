...
//        int moduleId = (int)request.getAttribute("moduleId");
        ...
        List<Card> cards = cardDao.get().getCardsByModuleId(moduleId);
        request.setAttribute("cards", cards);

        RequestDispatcher requestDispatcher = request.getRequestDispatcher("/card/getcards.jsp");
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
       ...
       // req.setAttribute("cardId", split[1]);
        Card card = cardDao.get().get(Integer.valueOf(split[1]));
        card.setId(Integer.valueOf(split[1]));
        req.setAttribute("card", card);
        RequestDispatcher requestDispatcher = req.getRequestDispatcher("/card/" + split[0] + ".jsp");
